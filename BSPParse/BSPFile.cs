...
namespace RRFull.BSPParse
{
    ...
    public class BSPFile
    {
        ...
        public bool IsVisible(SharpDX.Vector3 a, SharpDX.Vector3 b)
        {
            return IsVisible(new Vector3(a.X, a.Y, a.Z), new Vector3(b.X, b.Y, b.Z));
        }

        public bool IsVisible(SharpDX.Vector3 a, SharpDX.Vector3[] b)
        {
            for (int i = 0; i < b.Length; i++)
                if (IsVisible(new Vector3(a.X, a.Y, a.Z), new Vector3(b[i].X, b[i].Y, b[i].Z)))
                    return true;
            return false;
        }
        ...
        public bool Wrap(SharpDX.Vector3 a, SharpDX.Vector3 b, ItemDefinitionIndex _w)
        {
            return Wallbang(new Vector3(a.X, a.Y, a.Z), new Vector3(b.X, b.Y, b.Z), _w);
        }

        public bool Wallbang(Vector3 start, Vector3 end, ItemDefinitionIndex _wpid)
        {
            int _penetration = 3;
            ...
            while (stepCount > 0)
            {
                ...
                if (leaf.m_Area != -1)
                {
                    if ((leaf.m_Contents & SOLID_LEAF_FLAG) != 0)
                    {
                        if (_penetration <= 0)
                            return false;
                        //Console.WriteLine(_penetration);
                        _penetration--;
                    }
                }
        ...
        public bool IsVisibleEx(Vector3 start, Vector3 end)
        {
            var direction = end - start;
            var point = start;
            int stepCount = (int)direction.Length();
        ...
        private dleaf_t GetLeafForPoint(Vector3 point)
        {
            ...
            while (node >= 0)
            {
                ...
                d = Dot(point, pPlane.m_Normal) - pPlane.m_Distance;
        ...
        public float Dot(Vector3 _in, Vector3 other)
        {
            return (_in.X * other.X) + (_in.Y * other.Y) + (_in.Z * other.Z);
        }
        #endregion
    }
}